        ...
        <div className="">
          {
            isOpenAll ? (
              <button
                onClick={collapseAll}
                className="bg-red-500 text-xl text-yellow-100 rounded"
              >
                Collapse All
              </button>
            )
...

    {
      courseContent?.map((section, idx) => (
        <details key={idx}
          ...
          ref={(el) => detailRefs.current.push(el)}
          className="group overflow-hidden border border-solid border-richblack-600 bg-richblack-700 text-richblack-5 w-[60%] ml-5">
